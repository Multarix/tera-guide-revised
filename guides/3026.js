...
module.exports = (mod, extras) => {
	...
	function spawn_marker(out){
		if(!boss_ent) return;
		const boss_spawn = new spawn(boss_ent);
		let distance = 220;
		let caption = "IN";
		if(out){
			distance = 620;
	...
	function debuff_added(id){
		...
		timer1 = mod.setTimeout(() => {
			if(debuff != null){
				sendMessage("Debuff 20 seconds");
			}
		}, 70000);
		timer2 = mod.setTimeout(() => {
			if(debuff != null){
				mod.setTimeout(() => {
					sendMessage(`${debuff_messages[debuff % 2].message}`);
				}, 2000);
				sendMessage("Debuff 50 seconds");
			}
		}, 40000);
		timer3 = mod.setTimeout(() => {
			if(debuff != null){
				sendMessage("Warning! Debuff 15 seconds");
			}
		}, 75000);
		timer4 = mod.setTimeout(() => {
			if(debuff != null){
				sendMessage("Warning! Debuff 10 seconds");
			}
		}, 80000);
		timer5 = mod.setTimeout(() => {
			if(debuff != null){
				sendMessage("Warning! Debuff 5 seconds");
			}
		}, 85000);
		//
		if(blue){
			sendMessage(`${mech_messages[(qbacting + debuff + 1) % 2].message}`);
			spawn_marker((qbacting + debuff + 1) % 2);
		} else if(red){
			sendMessage(`${mech_messages[(qbacting + debuff) % 2].message}`);
	...
	function skilld_event(skillid){
		...
		// Fire/Ice debuff
			...
			if(player.isMe(event.target.toString()) && [30261701, 31261701].includes(event.id)){
				if(added && boss_ent){
					...
					spawnHandler({ // spawn teleport marker
		...
		if([212, 213, 214, 215].includes(skillid)){
			boss_ent = extras.ent;
			create.circle(false, 445, 0, 0, 8, 440, 8000);
			create.circle(false, 445, 0, 0, 4, 840, 8000);
		}
		if([212, 214].includes(skillid)){ // Fire claw (141, 142)
			boss_offset = 10;
			create.vector(553, 0, 0, 190, 840, 200, 8000);
			create.vector(553, 0, 0, 10, 840, 200, 8000);
		}
		if([213, 215].includes(skillid)){ // Ice claw (143, 144)
			boss_offset = -10;
			create.vector(553, 0, 0, 170, 840, 200, 8000);
			create.vector(553, 0, 0, 350, 840, 200, 8000);
		}
		if([213, 214].includes(skillid)){ // Ice inside
			mod.setTimeout(() => {
				if(debuff != null){
					sendMessage(`Ice inside (${qbacting_messages[qbacting].message}) | ${mech_messages[(qbacting + debuff + 1) % 2].message}`);
					spawn_marker((qbacting + debuff + 1) % 2);
				} else {
					sendMessage(`Ice inside (${qbacting_messages[qbacting].message})`);
				}
			}, 500);
			blue = true;
			red = false;
		...
		if([212, 215].includes(skillid)){ // Fire inside
			mod.setTimeout(() => {
				if(debuff != null){
					sendMessage(`Fire inside (${qbacting_messages[qbacting].message}) | ${mech_messages[(qbacting + debuff) % 2].message}`);
					spawn_marker((qbacting + debuff) % 2);
				} else {
					sendMessage(`Fire inside (${qbacting_messages[qbacting].message})`);
				}
			}, 500);
			blue = false;
			red = true;
		...
		if(!debuff_tracker_started){
			mod.hook("S_ABNORMALITY_BEGIN", 4, abnormality_change(true));
			mod.hook("S_ABNORMALITY_END", 1, abnormality_change(false));
	...
	for(const [key, value] of Object.entries(skills)){
		if(key.length === 5){
			skillGroup[`s-3026-1000-1${key}`] = value;
			skillGroup[`s-3026-1000-2${key}`] = value;
		} else {
