...
module.exports = (mod, extras, zone, quick) => {
	...
	if(extras.guides.includes(extras.lastLocation.toString())){ // if the last location is the same as a guide, try unloading it
		try {
			extras.active_guide = false;
			// delete require.cache[require.resolve(path.resolve(__dirname, `./guides/${zone}.js`))];
		} catch (e){ null; }
	}
	...
	if(!mod.settings.enabled) return; // return if the guide isn't even enabled
